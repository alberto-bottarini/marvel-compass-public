(function () {
  ...
  var slugs = window.__PAGE_SLUGS__ || { heroes: 'heroes', cards: 'cards' };
  var index = null;
  var activeIndex = -1;
  ...
  function render(results) {
    ...
    if (!results.length) {
      ...
      dropdown.innerHTML = results.slice(0, 12).map(function (r, i) {
        if (r.t === 'h') {
          return '<a class="search-item search-item--hero" href="/' + lang + '/' + slugs.heroes + '/' + r.slug + '/" data-idx="' + i + '">'
            + '<span class="search-item__type">' + escHtml(heroLabel()) + '</span>'
            + '<span class="search-item__name">' + escHtml(r.name) + '</span>'
            + '</a>';
        }
        return '<a class="search-item search-item--card" href="/' + lang + '/' + slugs.cards + '/' + r.code + '/" data-idx="' + i + '">'
